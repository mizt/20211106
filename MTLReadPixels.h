	...
	class MTLReadPixels {
		...
		public:
            ...
            MTLReadPixels(int w,int h,NSString *identifier=nil) {
				...
				NSString *metallib = [NSString stringWithFormat:COPY_SHADER_PATH,path,"copy.metallib"];
                //post("%s",[metallib UTF8String]);
					...
					if(size>0) {
						...
						dispatch_read(fd,size,dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,0),^(dispatch_data_t d,int e) {
							...
							if(!err) {
								...
								MTLTextureDescriptor *desc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA8Unorm width:this->_width height:this->_height mipmapped:NO];
